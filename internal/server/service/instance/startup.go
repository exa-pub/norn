...
// WatchLogs reads a JSONL log file and streams entries.
//   - If done is non-nil, tails until done closes (used for devcontainer up logs
//     where the LogWriter signals completion).
//   - If done is nil, tails indefinitely until ctx is cancelled (used for daemon
//     logs written by an external process).
func WatchLogs(ctx context.Context, path string, done <-chan struct{}) <-chan LogEntry {
	...
	go func() {
		...
		for {
			...
			if done != nil {
				// Finite mode: wait for writer to signal completion, then drain.
				select {
				case <-done:
					sc = bufio.NewScanner(f)
					for sc.Scan() {
				...
				// Infinite tail mode: poll until context cancelled.
				select {
				case <-ctx.Done():
					return
				case <-time.After(100 * time.Millisecond):
